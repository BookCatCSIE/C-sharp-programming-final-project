...
public class shoot1 : MonoBehaviour
{
    ...
    void Update()
    {
        if (gameObject.GetComponentInParent<tank_controller>().hp > 0)
        {
            if (i < 3)
            {
                i += Time.deltaTime;
            ...
            if (i > 1)
            {
                if (Input.GetMouseButtonDown(0))
                {
                    ...
                    bullet.GetComponent<Rigidbody>().AddForce(-(transform.forward-new Vector3(0,transform.forward.y,0) )* bullet_speed, ForceMode.Impulse);
                    AS.Play();
                }
            }
        }
